...
class TestAttributesProvider(unittest.TestCase):
    ...
    def test_gets_value_for_single_mbox(self):
        ...
        self.assertFalse(feature_a.get_value("feature-flag-a", "show_feature_x"))
        self.assertEqual(feature_a.get_value("feature-flag-a", "payment_gateway_version"), 2.3)
        self.assertEqual(feature_a.get_value("feature-flag-a", "customer_feedback_value"), 10)

        feature_b = AttributesProvider(target_response)

        self.assertEqual(feature_b.get_value("feature-flag-b", "purchase_experience"), "beta2")
